...
6. **Flag risk per slice.** Mark `Risk: high` when the slice touches: database schema or
   migrations, authentication/authorization, money movement, transformation of real client
   data, security-sensitive surfaces, or irreversible operations. Everything else is
   `Risk: standard`. This flag drives Phase 4's per-slice independent review gate
   (`workflow.per_slice_review` in risk-gated mode) -- an unflagged high-risk slice ships
   without fresh-eyes review until Phase 5, which is the most expensive time to catch an
   architecture-level mistake.
...
5. If `diagrams` AND `d2` both unavailable BUT Hyperframes available: render architecture via HTML+SVG (see `modules/hyperframes-renderer.md` arch-diagram composition)
